...
const EMPTY_SUMMARY = { critical: 0, high: 0, medium: 0, low: 0, total: 0, fixable: 0, accepted: 0 };
...
export function generateMarkdownReport(result: AuditResult): string {
  ...
  if (summary.critical > 0) {
    const fixable = result.vulnerabilities.filter((v) => v.severity === "critical" && v.fixVersion).length;
    lines.push(`| Critical | ${summary.critical} | ${fixable} | Fix immediately |`);
  }
  if (summary.high > 0) {
    const fixable = result.vulnerabilities.filter((v) => v.severity === "high" && v.fixVersion).length;
    lines.push(`| High | ${summary.high} | ${fixable} | Fix immediately |`);
  }
  if (summary.medium > 0) {
    const fixable = result.vulnerabilities.filter((v) => v.severity === "medium" && v.fixVersion).length;
    lines.push(`| Medium | ${summary.medium} | ${fixable} | Fix soon |`);
  }
  if (summary.low > 0) {
    const fixable = result.vulnerabilities.filter((v) => v.severity === "low" && v.fixVersion).length;
  ...
  lines.push(`\n**Total**: ${summary.total} vulnerabilities (${summary.fixable} auto-fixable)\n`);
  ...
  if (highPriority.length > 0) {
    ...
    for (const vuln of highPriority) {
      ...
      if (vuln.fixVersion) {
        ...
        if (vuln.isSemVerMajor) lines.push(`- **Warning**: This is a major version update`);
      ...
      if (vuln.url) lines.push(`- **Advisory**: ${vuln.url}`);
      lines.push("");
    }
  }
  ...
  if (fixable.length > 0 || unfixable.length > 0) {
    ...
    if (fixable.length > 0) {
      ...
      if (directFixes.length > 0) {
        ...
        for (const vuln of directFixes) lines.push(`npm update ${vuln.name}`);
        lines.push("");
      }
...
export function shouldFail(result: AuditResult, failOn: string): boolean {
  if (!result.success || !result.hasVulnerabilities) return false;
  const { summary } = result;
  switch (failOn) {
    case "critical":
