...
#   - php artisan test → pass/fail, counts
...
case "$COMMAND" in
  ...
  *"php artisan test"*|*"phpunit"*)
    if [[ "$EXIT_CODE" == "0" ]]; then
      ...
      log_event "test-pass" "\"summary\":\"$COUNTS\",\"tool\":\"php\""
    else
      FAIL_LINE=$(echo "$OUTPUT" | grep -oE '[0-9]+ (failed|error)' | head -1 | sed 's/"/\\"/g')
      log_event "test-fail" "\"summary\":\"$FAIL_LINE\",\"tool\":\"php\""
    fi
    ;;

  *"npm test"*|*"npm run test"*)
    if [[ "$EXIT_CODE" == "0" ]]; then
      log_event "test-pass" "\"tool\":\"npm\""
    else
      log_event "test-fail" "\"tool\":\"npm\""
    fi
    ;;
